...
const startwithdiamondLab: React.FC = () => {
  const { cart } = useCart();
  console.log(cart, 'dd');
  const metals = [
  ...
  const fetchProductsStep1 = async () => {
    try {
      const response = await AxiosInstance.get(
        `/Products/multiRingDesign/${selectedShape}/${selectedStyle}/${selectedMetal}/${selectedValue}/${JewelleryType}/${Category}/2`
      );
...
export default startwithdiamondLab;
