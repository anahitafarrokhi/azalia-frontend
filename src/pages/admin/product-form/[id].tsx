...
import ExistingImagesList, { /* type is inline above */ } from "@/components/ExistingImagesList";
import { AxiosInstance } from "@/utils/api";
import React, { useEffect, useState } from "react";
import Select from 'react-select';
import { useRouter } from "next/router";
export type ExistingImageDto = {
    id: number;
    imageUrl: string;
    isPrimary: boolean;
    imageType: number; // 1 = Images, 2 = Files (from your code)
};
const EditProduct = () => {
    ...
    // below your other useStates:
    const [existingImages, setExistingImages] = useState<ExistingImageDto[]>([]);
    ...
    const [images2, setImages] = useState<ImageData[]>([]);
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        form.append("Color", formData.colorInt);
        form.append("LabOrNat", formData.labOrNatInt);
        form.append("LabOrNatDesc", formData.labOrNatDesc);
        form.append("Ayar", formData.ayarInt);
        ...
        form.append("CountryMaker", formData.countryMakerInt);
        form.append("ProductGender", formData.productGenderInt);
        form.append("Style", formData.styleInt);
        form.append("Shape", formData.shapeInt);
        form.append("cut", formData.cutInt);
        form.append("DiamondColor", formData.diamondColorInt);
        form.append("Clarity", formData.clarityInt);
        form.append("BirthdayCategory", formData.birthdayCategoryInt);
        form.append("Gemstone", formData.gemstoneInt);
        ...
        // A) Build the kept lists for the server-side "ListView" fields
        const keptExistingImagesListView = existingImages
            .filter(i => i.imageType === 1); // only "images" for ImagesListView

        const keptExistingFilesListView = existingImages
            .filter(i => i.imageType === 2); // only "files" for FilesListView

        // OPTIONAL: ensure there is at most one primary among keptExistingImagesListView + new uploads.
        // If you already enforce this in UI, you can skip this guard.
        function ensureSinglePrimary(arr: { isPrimary: boolean }[]) {
            let found = false;
            for (const x of arr) {
                if (x.isPrimary) {
                    if (!found) found = true;
                    else x.isPrimary = false; // drop extras
                }
            }
        }
        ensureSinglePrimary(keptExistingImagesListView);

        // B) Append kept existing items to the *ListView* fields (NOT as CreateImageDto)
        keptExistingImagesListView.forEach((img, index) => {
            form.append(`ImagesListView[${index}].Id`, String(img.id));
            form.append(`ImagesListView[${index}].ImageUrl`, img.imageUrl);
            form.append(`ImagesListView[${index}].IsPrimary`, String(img.isPrimary));
            form.append(`ImagesListView[${index}].ImageType`, String(img.imageType)); // "1"
        });

        // FilesListView is nullable in your command — only append if you have any:
        if (keptExistingFilesListView.length > 0) {
            keptExistingFilesListView.forEach((img, index) => {
                form.append(`FilesListView[${index}].Id`, String(img.id));
                form.append(`FilesListView[${index}].ImageUrl`, img.imageUrl);
                form.append(`FilesListView[${index}].IsPrimary`, String(false)); // or keep a flag if you have one
                form.append(`FilesListView[${index}].ImageType`, String(img.imageType)); // "2"
            });
        }
        ...
        images2.forEach((file, index) => {
            console.log(String(index === primaryIndex), "dff")
            console.log(file, "fsdfdf")
            if (file instanceof File) {
                form.append(`Images[${index}].File`, file);
            }
            form.append(`Images[${index}].IsPrimary`, "False");  // ✅ true if primary
            form.append(`Images[${index}].ImageType`, "1");
        });
        console.log(images2, "images22")
    ...
    useEffect(() => {
        const fetchProductById = async () => {
            ...
            try {
                ...
                setFormData({
                    ...data,
                    caratWeight: data.weight,
                });
                if (data.jewelleryTypes) {
                    setSelectedJewelleryTypes(
                        data.jewelleryTypes.map((jt: any) => ({ value: jt.id, label: jt.name }))
                    );
                }
                if (data.images) {
                    const imgs: ExistingImageDto[] = data.images.map((img: any) => ({
                        id: img.id,
                        imageUrl: img.imageUrl,
                        isPrimary: img.isPrimary,
                        imageType: img.imageType, // 1 or 2
                    }));
    ...
    return (
        ...
        <form onSubmit={handleSubmit} className="max-w-7xl mx-auto p-4 grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            <h3 className="col-span-full text-2xl font-bold mb-4 mt-8">Edit a product</h3>
            ...
            <div className="col-span-full">
                <ExistingImagesList
                    ...
                    images={existingImages.filter(i => i.imageType === 2)}
                    onRemove={(id) => {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <select name="color" value={formData.colorInt} onChange={handleChange} className="border p-2 w-full">
                            <option value={0}>Choose one...</option>
                    ...
                    <div>
                        ...
                        <select name="ayarInt" value={formData.ayarInt} onChange={handleChange} className="border p-2 w-full">
                            <option value={0}>Choose one...</option>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <select name="labOrNatInt" value={formData.labOrNatInt} onChange={handleChange} className="border p-2 w-full">
                            <option value={0}>Choose one...</option>
                    ...
                    <div>
                        ...
                        <select name="countryMakerInt" value={formData.countryMakerInt} onChange={handleChange} className="border p-2 w-full">
                            <option value={0}>Choose one...</option>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <select name="productGenderInt" value={formData.productGenderInt} onChange={handleChange} className="border p-2 w-full">
                            <option value={0}>Choose one...</option>
                    ...
                    <div>
                        ...
                        <select name="styleInt" value={formData.styleInt} onChange={handleChange} className="border p-2 w-full">
                            <option value={0}>Choose one...</option>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <select name="clarityInt" value={formData.clarityInt} onChange={handleChange} className="border p-2 w-full">
                            <option value={0}>Choose one...</option>
                    ...
                    <div>
                        ...
                        <select name="diamondColorInt" value={formData.diamondColorInt} onChange={handleChange} className="border p-2 w-full">
                            <option value={0}>Choose one...</option>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <select name="cutInt" value={formData.cutInt} onChange={handleChange} className="border p-2 w-full">
                            <option value={0}>Choose one...</option>
                    ...
                    <div>
                        ...
                        <select name="shapeInt" value={formData.shapeInt} onChange={handleChange} className="border p-2 w-full">
                            <option value={0}>Choose one...</option>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <select name="gemstoneInt" value={formData.gemstoneInt} onChange={handleChange} className="border p-2 w-full">
                            <option value={0}>Choose one...</option>
                    ...
                    <div>
                        ...
                        <select name="birthdayCategoryInt" value={formData.birthdayCategoryInt} onChange={handleChange} className="border p-2 w-full">
                            <option value={0}>Choose one...</option>
                ...
                <ExistingImagesList
                    ...
                    images={existingImages.filter(i => i.imageType === 1)}
                    onRemove={(id) => {
                        setRemovedImageIds((prev) => [...new Set([...prev, id])]); // track deleted
                        setExistingImages((prev) => prev.filter((i) => i.id !== id)); // remove from UI
                    }}
                    onMakePrimary={(id) => {
                        setExistingImages((prev) => {
                            const clicked = prev.find((i) => i.id === id);
                            const isCurrentlyPrimary = !!clicked?.isPrimary;

                            // If clicking the current primary → unset it (no primary)
                            if (isCurrentlyPrimary) {
                                return prev.map((i) => (i.id === id ? { ...i, isPrimary: false } : i));
                            }

                            // Otherwise set this one as primary and unset the rest
                            return prev.map((i) => ({ ...i, isPrimary: i.id === id }));
                        });
                    }}
                ...
                <ImageUploader images={images2} setImages={setImages} primaryIndex={primaryIndex} setPrimaryIndex={setPrimaryIndex} />
