...
import SampleProduct from '@/assets/images/sampleProduct.webp';
...
const DiamondShapes: React.FC = () => {
    ...
    let { JewelleryType, Shape, Category }: any = router.query;
    const [selectedShapeStep2, setSelectedShapeStep2] = useState(Shape);
    ...
    const steps = [
        { stepNumber: 1, title: "Choose a", subtitle: "DIAMOND" },
        { stepNumber: 2, title: "Choose a", subtitle: "SETTING" },
    ...
    const fetchProductsStep1 = async () => {
        console.log('12', selectedShape, selectedStyle, selectedMetal, selectedValue, JewelleryType, Category);
        try {
    ...
    const fetchProductsStep2 = async () => {
        if (selectedShapeStep2) {
            try {
                console.log('111', selectedShapeStep2, debouncedMinInput, debouncedMaxInput, debouncedlocalMin, debouncedlocalMax, debouncedMinRangeValue, debouncedMaxRangeValue, titleValue, JewelleryType);
    ...
    return (
        <>
            ...
            {/* Overlay + Slide-in Modal (no framer-motion) */}
            {showFilters && (
                <>
                    {/* Background Overlay */}
                    <div
