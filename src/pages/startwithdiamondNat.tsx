...
import CaratRange from "@/components/CaratRange";
import DiamondList from "@/components/DiamondList";
import CustomizationOptions from "@/components/CustomizationOptions";
import designedring from "@/assets/images/designedring.webp";
...
const startwithdiamondNat: React.FC = () => {
  const { cart } = useCart();
  console.log(cart, 'dd');
  const metals = [
  ...
  const handleNextStep = () => {
    setCurrentStep(2);
    // Add your navigation or functionality for the next step here
  };
  const renderOptions = (options: Array<any>, selected: any | null, colNum: any, setSelected: (id: any) => void) => {
    return (
      <div className={`${colNum ? 'md:grid-cols-5' : 'md:grid-cols-2'} grid grid-cols-2 md:grid-cols-2 gap-4`}>
        {options.map((option) => (
          <div
            key={option.id}
            className={`p-4 border rounded-lg text-center cursor-pointer transition ${selected === option.id ? "border-black" : "border-gray-300"
              }`}
            onClick={() => setSelected(option.id)}
          >
            <Image src={option.icon} className="text-2xl mx-auto" alt="" width={100} height={100} />
            <div className="mt-2 font-medium text-xs">{option.label}</div>
          </div>
        ))}
      </div>
    );
  ...
  useEffect(() => {
    console.log('1', selectedShapeStep2, debouncedMinInput, debouncedMaxInput, debouncedlocalMin, debouncedlocalMax, debouncedMinRangeValue, debouncedMaxRangeValue, titleValue, JewelleryType);
    fetchProductsStep2();
  ...
  const fetchProductsStep1 = async () => {
    try {
      const response = await AxiosInstance.get(
        `/Products/multiRingDesign/${selectedShape}/${selectedStyle}/${selectedMetal}/${selectedValue}/${JewelleryType}/${Category}/1`
      );
  ...
  const fetchProductsStep2 = async () => {
    if (currentStep === 2) {
      console.log(selectedShapeStep2, debouncedMinInput, debouncedMaxInput, debouncedlocalMin, debouncedlocalMax, debouncedMinRangeValue, debouncedMaxRangeValue, titleValue, JewelleryType, 'salam');
  ...
  useEffect(() => {
    if (products && products.length > 0) {
      ...
      setSelectedProduct(null); // 🔁 Set to null if no products available
    }
  }, [products]);

  console.log('dff', selectedDiamond);
  return (
    <>
...
export default startwithdiamondNat;
